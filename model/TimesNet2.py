...
class TimeBlock(nn.Module):
    ...
    def forward(self,x):
        ...
        B,L,C = x.size()
        periods,per_weight = periodforacf(season)
        res = []
        # print(periods)
        # resahpe (bs -1 period d_model )
        for i in range(self.config.k2):
            period = periods[i]
            # padding
        ...
        per_weight = per_weight.unsqueeze(0).unsqueeze(0).unsqueeze(0).repeat(B,self.config.seq_len+self.config.pred_len,C,1)
        res = torch.sum(per_weight*res,-1)
        res = res + x
        return res,periods
def periodforacf(x):
    xf = torch.fft.rfft(x,dim = 1)
    xf = abs(xf).mean(0).mean(-1) ** 2
    acf1 = torch.fft.ifft(xf)
    acf1 = acf1/acf1[1]
    acf1[1] = 0
    acf1 = abs(acf1)
    peak_indices = []
    window_size = 3  # 设置窗口大小，用于判断局部最大值

    for i in range(window_size, len(acf1) - window_size):
        window = acf1[i - window_size: i + window_size + 1]
        if acf1[i] == window.max() and i not in peak_indices:
            peak_indices.append(i)
        if len(peak_indices) >= 5:  # 只保留前 3 个主要峰值
            break
    return peak_indices, acf1[peak_indices]

def FFT_for_periods(x,k):
    xf = torch.fft.rfft(x, dim=1)
    # find period by amplitudes
    # abs 是对复数取模 192 得到 96个频率分量
    fl = torch.fft.ifft(abs(xf)[0,:,0] ** 2)
    fl = fl.cpu().detach().numpy()
    fl[0] = 0
    frequency_list = abs(xf).mean(0).mean(-1)
    frequency_list = frequency_list.cpu().detach().numpy()  # 如果 frequency_list 是 PyTorch tensor

    # 创建 x 轴数据
    x = np.arange(len(frequency_list))

    # 绘制图形
    plt.figure(figsize=(10, 5))
    plt.subplot(2,1,1)
    plt.plot(x, frequency_list)
    plt.title('Frequency List Visualization')
    plt.xlabel('Index')
...
class TimesNet2(nn.Module):
    def __init__(self,config):
        super(TimesNet2, self).__init__()
        self.config = config
        ...
        self.linear = nn.Linear(96, config.d_model)
